...
impl FireworksSTT {
    ...
    /// Transcribe streaming audio using speech events for triggering
    ...
    pub async fn transcribe_stream_with_events(
        ...
        // Spawn a dedicated task for monitoring speech events and managing connection
        let _sender_handle = tokio::spawn(async move {
            let mut in_speech = false;
            let mut final_checkpoint_sent = false;

            log::info!("STT: Waiting for speech events...");
            ...
            loop {
                ...
                match events_receiver.recv().await {
                            ...
                            SpeechEvent::StoppedSpeaking => {
                                if in_speech {
                                        ...
                                        write.send(Message::Text(checkpoint_msg.into())).await
                                    {
                                        ...
                                        log::info!("STT: Final checkpoint sent successfully");
                                        final_checkpoint_sent = true;
                                        in_speech = false;
                                    }
                                }
                    ...
                    Err(broadcast::error::RecvError::Lagged(skipped)) => {
                        log::warn!("STT: Events receiver lagged, skipped {} events", skipped);
                        continue;
                    }
                    Err(broadcast::error::RecvError::Closed) => {
                        log::info!("STT: Events stream closed");
                        if in_speech {
                            // Send final checkpoint before closing
                            let final_checkpoint = json!({"checkpoint_id": "final"});
                            let checkpoint_msg = serde_json::to_string(&final_checkpoint).unwrap();
                            let _ = write.send(Message::Text(checkpoint_msg.into())).await;
                            final_checkpoint_sent = true;
                        }
                        break;
                    }
            ...
            log::info!("STT: Closing events-based STT sender");
            let _ = write.close().await;
        });
        ...
        log::info!("STT: Starting to listen for server responses (events-based)");

        // Process messages until we get final checkpoint or timeout
        loop {
            let timeout_duration = server_timeout;
            match tokio::time::timeout(timeout_duration, read.next()).await {
                    ...
                    match msg {
                        Message::Text(text) => {
                                ...
                                serde_json::from_str::<serde_json::Value>(text.as_str())
                            {
                                ...
                                if let Some(checkpoint_id) = response.get("checkpoint_id") {
                                ...
                                if let Some(segments) =
                                    response.get("segments").and_then(|s| s.as_array())
                                {
                                    for segment in segments {
                                            ...
                                            segment.get("text").and_then(|t| t.as_str())
                                        {
                                            if !text.trim().is_empty() {
                                                log::debug!("STT segment: '{}'", text.trim());
                                                final_transcript.push_str(text.trim());
                ...
                Err(_) => {
                    ...
                    if elapsed > server_timeout {
                        log::warn!("STT: Server timeout after {:?}", elapsed);
                        break;
                    }
                }
            }
        ...
        final_transcript = final_transcript.trim().to_string();

        if let Ok(stats) = stats.lock() {
            log::info!(
                "STT: Transcription completed in {:.2}ms, processed {} messages",
        ...
        if final_transcript.is_empty() {
            log::warn!("STT: No transcription received");
            return Err(STTError::NoTranscription);
        }

        log::info!("STT: Final result: '{}'", final_transcript);
        Ok(final_transcript)
    }
}
