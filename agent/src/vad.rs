...
impl VadProcessor {
    ...
    pub fn process_chunk(
        &mut self,
        samples: &[f32; 512],
    ) -> Result<crate::types::AudioEvent, VadError> {
        ...
        let audio_event = match (self.current_state.clone(), has_speech) {
            ...
            (VadState::Silence, true) => {
                ...
                if self.state_duration_ms >= self.config.speech_start_threshold_ms {
                    ...
                    crate::types::AudioEvent::Audio // Treat as ongoing until confirmed
                }
            }
            (VadState::Silence, false) => {
                // Still in silence
                crate::types::AudioEvent::Audio // During transcription, silence is still "audio"
            }
            // Currently in speech
