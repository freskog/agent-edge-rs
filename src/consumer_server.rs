...
impl ConsumerServer {
    ...
    fn detection_thread(
        ...
        // Initialize wakeword model
        {
            ...
            if model_guard.is_none() {
                ...
                match WakewordModel::new_with_model_path(
                    ...
                    Ok(model) => {
                        *model_guard = Some(model);
                        log::info!(
                            "✅ Wakeword model loaded with {} wake models",
        ...
        unsafe {
            ...
            if ret == 0 {
                log::info!("Detection thread pinned to core 1");
            } else {
                log::warn!(
                    "Failed to pin detection thread to core 1: {}",
        ...
        while !should_stop.load(Ordering::SeqCst) {
            ...
            if let Some(ref chunk_data) = audio {
                ...
                if !samples.is_empty() {
                    ...
                    let (wakeword_event, last_ww_max_conf) = {
    ...
    fn process_wakeword_detection_standalone(
                ...
                Ok(predictions) => {
                    ...
                    for (model_name, confidence) in predictions {
                        ...
                        if confidence >= threshold {
                            ...
                            if should_debounce {
                                log::debug!(
                                    "🔇 [Detection] Wake word '{}' debounced (confidence {:.6}) - last was {:.1}ms ago",
                                    model_name,
                                    confidence,
        ...
        // No wake fired; return the peak for near-miss logging.
        Ok((None, max_conf))
    }
