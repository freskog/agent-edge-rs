...
impl MelSpectrogramModel {
    ...
    pub fn new(model_path: &str) -> Result<Self> {
        ...
        let model = Box::leak(Box::new(Model::new(&config.model_path).map_err(|e| {
            EdgeError::ModelLoadError(format!("Failed to load model: {}", e))
        })?));

        // Attempt to create the interpreter, first with XNNPACK (if enabled) then fall back
        let mut interpreter_res = Interpreter::new(model, Some(options.clone()));

        // XNNPACK is now enabled with pthreadpool linking

        let interpreter = interpreter_res.map_err(|e| {
    ...
    pub fn predict(&mut self, audio_samples: &[f32]) -> Result<Vec<f32>> {
...
mod tests {
    ...
    #[test]
    fn test_melspec_model_creation_xnnpack() {
        let mut options = Options::default();
        // XNNPACK is now enabled with pthreadpool linking
        let model = Model::new("models/melspectrogram.tflite").unwrap();
        let interpreter = Interpreter::new(&model, Some(options));
        assert!(interpreter.is_ok());
    }

    #[test]
    fn test_melspec_model_creation_no_xnnpack() {
