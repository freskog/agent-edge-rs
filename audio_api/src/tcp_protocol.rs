...
impl TryFrom<u8> for MessageType {
    ...
    fn try_from(value: u8) -> Result<Self, Self::Error> {
        match value {
            ...
            0x02 => Ok(MessageType::PlayAudio),
            0x03 => Ok(MessageType::EndStream),
            0x04 => Ok(MessageType::AbortPlayback),
            0x10 => Ok(MessageType::AudioChunk),
            0x11 => Ok(MessageType::PlayResponse),
            0x12 => Ok(MessageType::EndStreamResponse),
            0x13 => Ok(MessageType::AbortResponse),
...
pub enum Message {
    // Client → Server
    SubscribeAudio,
    PlayAudio { stream_id: String, audio_data: Vec<u8> },
    EndStream { stream_id: String },
    AbortPlayback { stream_id: String },
    
    // Server → Client
    AudioChunk { audio_data: Vec<u8>, timestamp_ms: u64 },
    PlayResponse { success: bool, message: String },
    EndStreamResponse { success: bool, message: String, chunks_played: u32 },
    AbortResponse { success: bool, message: String },
...
impl Message {
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>, ProtocolError> {
        ...
        match self {
            ...
            Message::PlayAudio { stream_id, audio_data } => {
            ...
            Message::AudioChunk { audio_data, timestamp_ms } => {
            ...
            Message::EndStreamResponse { success, message, chunks_played } => {
                payload.push(if *success { 1 } else { 0 });
    ...
    pub fn from_bytes(message_type: MessageType, payload: &[u8]) -> Result<Self, ProtocolError> {
        ...
        match message_type {
            ...
            MessageType::AudioChunk => {
                ...
                reader = &reader[8..];
                let audio_data = read_bytes(&mut reader)?;
                Ok(Message::AudioChunk { audio_data, timestamp_ms })
            }
            ...
            MessageType::EndStreamResponse => {
                ...
                let success = reader[0] != 0;
                let chunks_played = u32::from_le_bytes([reader[1], reader[2], reader[3], reader[4]]);
                reader = &reader[5..];
                let message = read_string(&mut reader)?;
                Ok(Message::EndStreamResponse { success, message, chunks_played })
            }
